 ...
 */
const useTransitionSuspense = ({ delay }: { delay: number }) => {
  ...
  const suspense = useCallback(
    ({ children }: any) =>
      DelayedSuspense(isPending, fallback.current, children),
    [isPending]
  );
...
const FallbackComponent = ({ delay, setIsPending, setIsFullfilled }: any) => {
  useEffect(() => {
    setIsPending(true);
...
const DelayedSuspense = (isPending: any, fallback: any, children: any) => {
  return (
